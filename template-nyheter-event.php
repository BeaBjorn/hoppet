...
<?php
/*
Template Name: nyheter-event-info
*/
?>
...
<?php get_header(); ?>
        ...
        <?php
        /* This php-block uses query_posts to only display posts from the category "nyheter-event-info" 
        and is set to display one post only. */
        query_posts('category_name=nyheter-event-info&posts_per_page=1');
            if(have_posts()){
                while(have_posts()){
                    the_post();
                    ?>
        ...
        <section class="green-blocks-sec">
            ...
            wp_reset_query();
            /* This php-block uses query_posts to only display posts from the category "nyheter-event-poster" 
            and is set to display one post only.  */   
            query_posts('category_name=nyheter-event-poster');
